// Composer attachments: File → Attachment (image/PDF → data URL, text → inline text).
// Ported from the old src/attach.ts — same thresholds, same shapes (the WS contract).
...
/** Read a File into an Attachment. Returns null for unsupported types or oversized files. */
...
// Identify an attachment by name + payload size so duplicates collapse to one chip.
const attKey = (a: Attachment) =>
  a.kind === "text"
    ? `t:${a.name}:${a.text?.length ?? 0}`
    : `${a.kind[0]}:${a.name}:${a.data_url?.length ?? 0}`;

export const mergeAttachments = (cur: Attachment[], add: Attachment[]): Attachment[] => {
